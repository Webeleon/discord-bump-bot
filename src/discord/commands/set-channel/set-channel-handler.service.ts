...
export class SetChannelHandler implements ICommandService {
  ...
  async execute(message: Message): Promise<void> {
    Logger.debug(`Setting channel ${message.channel.id} for server ${message.guild.id}`);

    // Any restrictrion?

    await this.serverService.setChannel(message.guild.id, message.channel.id);
    message.reply(`You've just set the advertisement channel to the current channel`);
  }
}
